...
void common_sampler_accept(struct common_sampler * gsmpl, llama_token token, bool is_generated) {
    ...
    if (gsmpl->rbudget && is_generated) {
        ...
        // If reasoning just finished, replay the matched end sequence through grammar.
        // This allows grammar triggers that depend on that end sequence to activate.
        ...
        if (gsmpl->grmr && !accept_grammar && is_done) {
            const std::vector<llama_token> * end_seq = common_reasoning_budget_get_matched_end(gsmpl->rbudget);
            if (end_seq) {
